...
bool Input::waitForCompletion(int userInput, float timer) {
...
bool Input::gameOn() {
  ...
  if (digitalRead(ON) == LOW) { //map pin to on/off switch
    return true;
  }
...
void Input::begin() {
  ...
  Wire1.begin(17,18); //GPIO 47 = Data, GPIO 48 = Clk for ACCEL
...
bool Input::checkShake() {
  ...
  if (!isfinite(accTotal) || accTotal < 0.5f) {
...
void Input::restartMPU() {
  ...
  for (int i = 0; i < 9 && digitalRead(17) == LOW; i++) {
    digitalWrite(18, LOW);
    delayMicroseconds(5);
